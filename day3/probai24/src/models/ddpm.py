...
class DDPM(torch.nn.Module):
    ...
    def get_coefs(
        ...
        if type == "linear":
            ...
            betas = torch.linspace(0.0001, 0.02, N)
            alphas = 1 - betas
    ...
    def _q_mean(
        ...
        # raise NotImplementedError
        _mean = torch.sqrt(self.alpha_bars(t))
        mean_out = broadcast_like(_mean, z) * z
        return mean_out
    ...
    def _q_std(
        ...
        # raise NotImplementedError
        _std = torch.sqrt(1- self.alpha_bars(t))
        std_out = broadcast_like(_std, z)
        return std_out
    ...
    def _losses(
        ...
        # Compute squared error and sum along dim axis 
        squared_error = torch.square(epsilon_pred - epsilon).sum(dim=1)
    ...
    def _p_mean(
        ...
        # raise NotImplementedError
        mean = 1 / torch.sqrt(alpha) * (z - beta/torch.sqrt(1-alpha_bar) * epsilon_pred)
        return mean
